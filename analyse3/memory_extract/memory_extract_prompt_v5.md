# Memory Extraction Prompt v5 — Empty Memory Allowed
...
- user_memories：完整记录用户明确表达的事实和经历。不同事实分别记录，保留其原始具体信息，不合并成范围或概括；每条只记录事实本身，不添加额外解释。
...
user_memories 的保留标准：用户提问本身不等于用户记忆，AI 的回答也不自动成为用户记忆。只有用户明确表达的事实和经历才能进入 user_memories。

如果 topic_context 和 user_memories 都没有任何值得保留的内容，直接返回：

{
  "status": "empty"
}

此时不填写 topic 和 summary。

整理要求：

- 合并 topic_context 中重复或高度相似的内容；
- user_memories 只合并完全重复的事实，保留不同时间或不同数值的独立事实；
- 为每项内容生成一个准确、简洁的自然语言 type 标签，type 不使用固定枚举；
- 只根据输入对话内容提取和概括。

有可保留内容时，输出一个 JSON 对象，字段必须为 topic、summary、topic_context、user_memories。
...
4. all facts and experiences explicitly expressed by the user.
...
- user_memories: Preserve all facts and experiences explicitly expressed by the user. Record distinct facts separately, retain their original concrete information, and do not merge them into ranges or generalizations. Write only each fact itself, without extra explanation.
...
Retention rule for user_memories: a user question by itself is not a user memory, and an assistant answer does not automatically become a user memory. Put only facts and experiences explicitly stated by the user in user_memories.

When both topic_context and user_memories contain no content worth retaining, return exactly:

{
  "status": "empty"
}

In this case, leave out topic and summary.
...
When retainable content exists, return one JSON object with exactly these fields: topic, summary, topic_context, user_memories.
...
{
  "type": "a type label chosen from the content",
  "content": "a fact or experience explicitly stated by the user"
}
```
