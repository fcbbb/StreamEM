...
6. 正常压缩逐层进行；旁路只允许 L1→L2、L2→L3，并直接融合到相邻高层 owner，不让低层跨级写入高层。
...
当前旁路作为默认生命周期的一部分，只允许相邻层路由：`L1 → L2`、`L2 → L3`。
候选限定为精确的下一层；命中唯一 owner 后，以当前低层 memory 的语义粒度融合更新高层，
并将高层 `last_mentioned_at` 更新为当前 checkpoint 日期。没有 owner、路由不确定或融合失败时，
回退到普通的低层创建/晋升流程。
