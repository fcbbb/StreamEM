...
DEFAULT_SYSTEM_PROMPT = """You are segmenting a conversation between a user and an AI assistant into continuous conversation events.
...
DEFAULT_USER_TEMPLATE = "Please segment the following conversation:\n\n{{units_json}}"
...
def load_prompt(prompt_dir: Path, version: str | None = None) -> PromptSpec:
    """Load a versioned prompt from prompts/<version>/.

    Keeping prompt text outside Python makes prompt iteration a data change:
    copy a directory, edit system.txt, and run with the new version.
    """
    system_path = prompt_dir / "system.txt"
    if not system_path.exists():
        raise FileNotFoundError(f"prompt system file not found: {system_path}")
    user_path = prompt_dir / "user_template.txt"
    system_prompt = system_path.read_text(encoding="utf-8").strip()
    user_template = (user_path.read_text(encoding="utf-8").strip()
                     if user_path.exists() else DEFAULT_USER_TEMPLATE)
    resolved_version = version or prompt_dir.name
    if not system_prompt:
        raise ValueError(f"prompt system file is empty: {system_path}")
    if "{{units_json}}" not in user_template:
        raise ValueError(f"user template must contain {{units_json}}: {user_path}")
    return PromptSpec(resolved_version, prompt_dir, system_prompt, user_template)
...
def make_user_prompt(units: list[dict[str, Any]], template: str = DEFAULT_USER_TEMPLATE) -> str:
    return template.replace("{{units_json}}", json.dumps(units, ensure_ascii=False, indent=2))
